...
TEST_GROUP(tests_div32f);
TEST_SETUP(tests_div32f) {}
TEST_TEAR_DOWN(tests_div32f) {}
 ...
 * */
void create_div32f_vecs(nmpps32f* Divided, nmpps32f bgnDivided, nmpps32f stepDivided,
...
const nmpps32f div32f_critical_error = 1e-04;
...
nmppsStatus test_div32f_diap(nmpps32f bgnDivided, nmpps32f stepDivided,
	...
	create_div32f_vecs(Divided, bgnDivided, stepDivided,
	...
	for (i=0; i<count; i++){
		...
		if (er > max_errf) {
			max_errf = er;
			if (max_errf > div32f_critical_error) {
				return i+1;
			}
		}
	}
...
TEST(tests_div32f, nmppsDiv_32f_small_vecs) {
	TEST_ASSERT_EQUAL(nmppsStsNoErr, test_div32f_diap(0, 0.133377789,
			              1000, 777.456,  1));
	TEST_ASSERT_EQUAL(nmppsStsNoErr, test_div32f_diap(0, 0.133377789,
			              1000, 777.456,  2));
	TEST_ASSERT_EQUAL(nmppsStsNoErr, test_div32f_diap(0, 0.133377789,
...
//Обычные значения
TEST(tests_div32f, nmppsDiv_32f_Normal) {
	nmppsStatus stat = nmppsStsNoErr;
	stat = test_div32f_diap(0, 0.133377789,
...
//Большие значения
TEST(tests_div32f, nmppsDiv_32f_Big) {
	nmppsStatus stat = nmppsStsNoErr;
	stat = test_div32f_diap(1.7789e+38, 13.337e+35,
...
TEST(tests_div32f, nmppsDiv_32f_Small) {
	nmppsStatus stat = nmppsStsNoErr;
	stat = test_div32f_diap(3.3333e-38, 1.3337e-35,
            1e-19, 777.456e+19,  COUNT_ITERATION);
    TEST_ASSERT_EQUAL(nmppsStsNoErr, stat);
}
...
TEST(tests_div32f, nmppsDiv_32f_check_answer) {
...
TEST(tests_div32f, nmppsDiv_32f_check_rewrite) {
	int i,k;
	//nmppsStatus stat;
...
TEST(tests_div32f, nmppsDiv_32f_subnormal) {
	float divided[29] = {
...
TEST_GROUP_RUNNER(tests_div32f){

    RUN_TEST_CASE(tests_div32f, nmppsDiv_32f_small_vecs);
    RUN_TEST_CASE(tests_div32f, nmppsDiv_32f_Normal);
    RUN_TEST_CASE(tests_div32f, nmppsDiv_32f_Small);
    RUN_TEST_CASE(tests_div32f, nmppsDiv_32f_Big);
    RUN_TEST_CASE(tests_div32f, nmppsDiv_32f_check_answer);
    RUN_TEST_CASE(tests_div32f, nmppsDiv_32f_check_rewrite);
    RUN_TEST_CASE(tests_div32f, nmppsDiv_32f_subnormal);
