...
TEST_GROUP(tests_divC32f);
TEST_SETUP(tests_divC32f) {}
TEST_TEAR_DOWN(tests_divC32f) {}
...
void create_divC32f_vecs(nmpps32f* Divided, nmpps32f bgnDivided, nmpps32f stepDivided,
...
nmppsStatus test_divC32f_diap(nmpps32f bgnDivided, nmpps32f stepDivided,
	...
	for (k=0; k < count; k++) {
		...
		create_divC32f_vecs(Divided, bgnDivided, stepDivided,
						bgnDivisor, kd, count);
		//Производим рассчеты
...
TEST(tests_divC32f, nmppsDivC_32f_small_vecs) {
	TEST_ASSERT_EQUAL(nmppsStsNoErr, test_divC32f_diap(0, 0.133377789,
			              1000, 777.456,  1));
	TEST_ASSERT_EQUAL(nmppsStsNoErr, test_divC32f_diap(0, 0.133377789,
			              1000, 777.456,  2));
	TEST_ASSERT_EQUAL(nmppsStsNoErr, test_divC32f_diap(0, 0.133377789,
...
//Обычные значения
TEST(tests_divC32f, nmppsDivC_32f_Normal) {
	nmppsStatus stat = nmppsStsNoErr;
	stat = test_divC32f_diap(0, 0.133377789,
...
//Большие значения
TEST(tests_divC32f, nmppsDivC_32f_Big) {
	nmppsStatus stat = nmppsStsNoErr;
	stat = test_divC32f_diap(1.7789e+38, 13.337e+35,
...
TEST(tests_divC32f, nmppsDivC_32f_Small) {
	nmppsStatus stat = nmppsStsNoErr;
	stat = test_divC32f_diap(3.3333e-38, 1.3337e-35,
            1e-19, 777.456e+19,  COUNT_ITERATION/10);
    TEST_ASSERT_EQUAL(nmppsStsNoErr, stat);
}
...
TEST(tests_divC32f, nmppsDivC_32f_check_answer) {
...
TEST(tests_divC32f, nmppsDivC_32f_check_rewrite) {
	int i,k;
	//nmppsStatus stat;
...
TEST(tests_divC32f, nmppsDivC_32f_subnormal) {
	float divided[29] = {
...
TEST_GROUP_RUNNER(tests_divC32f){
    RUN_TEST_CASE(tests_divC32f, nmppsDivC_32f_small_vecs);
    RUN_TEST_CASE(tests_divC32f, nmppsDivC_32f_Normal);
    RUN_TEST_CASE(tests_divC32f, nmppsDivC_32f_Big);
    RUN_TEST_CASE(tests_divC32f, nmppsDivC_32f_Small);
    RUN_TEST_CASE(tests_divC32f, nmppsDivC_32f_check_answer);
    RUN_TEST_CASE(tests_divC32f, nmppsDivC_32f_check_rewrite);
    RUN_TEST_CASE(tests_divC32f, nmppsDivC_32f_subnormal);
