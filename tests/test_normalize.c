...
TEST_GROUP(tests_normalize);
TEST_SETUP(tests_normalize) {}
TEST_TEAR_DOWN(tests_normalize) {}
...
//Обычные значения
TEST(tests_normalize, nmppsNormalize_64f_Calculation) {
...
TEST(tests_normalize, nmppsNormalize_64fc_Calculation) {
...
TEST(tests_normalize, nmppsNormalize_64f_check_answer) {
	nmppsStatus stat;
...
TEST(tests_normalize, nmppsNormalize_64fc_check_answer) {
...
TEST(tests_normalize, nmppsNormalize_64f_check_rewrite) {
	int i;
	dblint_t t, t1;
...
TEST(tests_normalize, nmppsNormalize_64fc_check_rewrite) {
	int i;
	dblint_t t, t1, t2;
	nmpps64fc* inputfc = (nmpps64fc*)input;
	nmpps64fc* outputfc = (nmpps64fc*)output;
...
TEST_GROUP_RUNNER(tests_normalize){
    RUN_TEST_CASE(tests_normalize, nmppsNormalize_64f_Calculation);
    RUN_TEST_CASE(tests_normalize, nmppsNormalize_64f_check_answer);
    RUN_TEST_CASE(tests_normalize, nmppsNormalize_64f_check_rewrite);

    RUN_TEST_CASE(tests_normalize, nmppsNormalize_64fc_Calculation);
    RUN_TEST_CASE(tests_normalize, nmppsNormalize_64fc_check_answer);
    RUN_TEST_CASE(tests_normalize, nmppsNormalize_64fc_check_rewrite);
