...
TEST_GROUP(tests_blackmanStd32fc);
TEST_SETUP(tests_blackmanStd32fc) {}
TEST_TEAR_DOWN(tests_blackmanStd32fc) {}
 ...
 * */
void create_blackmanStd32fc_vecs(nmpps32fc* in, nmpps32fc* out, unsigned int len,
					  nmpps32fc bgn, nmpps32fc step)
{
	nmpps32f w, tmp, tmp1;
	nmpps32f alpha = -0.16f;
	int i;
 ...
 * */
nmppsStatus test_blackmanStd32fc_diap( nmpps32fc bgn, nmpps32fc step, int count){
	nmpps32fc in[count];
	nmpps32fc res[count];
	nmpps32fc kd[count];
	nmpps32f er, tmp;
	...
	create_blackmanStd32fc_vecs(in, kd, count, bgn, step);
	//Расчет проверяемых значений
	stat = nmppsWinBlackmanStd_32fc(in, res, count);
...
TEST(tests_blackmanStd32fc, nmppsblackmanStd32fc_check_answer){
	nmpps32fc data[1];
	TEST_ASSERT_EQUAL(nmppsStsNullPtrErr, nmppsWinBlackmanStd_32fc(NULL, data, 3));
	TEST_ASSERT_EQUAL(nmppsStsNullPtrErr, nmppsWinBlackmanStd_32fc(data, NULL, 3));

	TEST_ASSERT_EQUAL(nmppsStsSizeErr, nmppsWinBlackmanStd_32fc(data, data, 2));
	TEST_ASSERT_EQUAL(nmppsStsSizeErr, nmppsWinBlackmanStd_32fc(data, data, 1));
	TEST_ASSERT_EQUAL(nmppsStsSizeErr, nmppsWinBlackmanStd_32fc(data, data, 0));
	TEST_ASSERT_EQUAL(nmppsStsSizeErr, nmppsWinBlackmanStd_32fc(data, data, -1));

}

TEST(tests_blackmanStd32fc, nmppsblackmanStd32fc_check_rewrite){
	int i,k;
	...
	for(i=3; i< (sizeof(data)/sizeof(nmpps32fc)) - 2; i++){
		nmppsWinBlackmanStd_32fc(&data[1], &out[1], i);
...
TEST(tests_blackmanStd32fc, nmppsblackmanStd32fc_small_vecs){
	nmpps32fc bgn, step;
	bgn.re = 0; bgn.re = 0;
	step.im = 1; step.im = 1;
	TEST_ASSERT_EQUAL(nmppsStsNoErr, test_blackmanStd32fc_diap(bgn, step, 3));
	step.im = 0.1; step.im = 0.1;
	TEST_ASSERT_EQUAL(nmppsStsNoErr, test_blackmanStd32fc_diap(bgn, step, 4));
	step.im = 0.01; step.im = 0.01;
	TEST_ASSERT_EQUAL(nmppsStsNoErr, test_blackmanStd32fc_diap(bgn, step, 5));
	step.im = 0.001; step.im = 0.001;
	TEST_ASSERT_EQUAL(nmppsStsNoErr, test_blackmanStd32fc_diap(bgn, step, 32));
	step.im = 0.0001; step.im = 0.0001;
	TEST_ASSERT_EQUAL(nmppsStsNoErr, test_blackmanStd32fc_diap(bgn, step, 33));
	step.im = 0.00001; step.im = 0.00001;
	TEST_ASSERT_EQUAL(nmppsStsNoErr, test_blackmanStd32fc_diap(bgn, step, 64));
	step.im = 0.000001; step.im = 0.000001;
	TEST_ASSERT_EQUAL(nmppsStsNoErr, test_blackmanStd32fc_diap(bgn, step, 65));
}



TEST(tests_blackmanStd32fc, nmppsblackmanStd32fc_calculation){
	nmpps32fc bgn, step;
	...
	TEST_ASSERT_EQUAL(nmppsStsNoErr, test_blackmanStd32fc_diap(bgn, step, COUNT_ITERATION));
...
TEST_GROUP_RUNNER(tests_blackmanStd32fc){
    RUN_TEST_CASE(tests_blackmanStd32fc, nmppsblackmanStd32fc_check_answer);
    RUN_TEST_CASE(tests_blackmanStd32fc, nmppsblackmanStd32fc_small_vecs);
    RUN_TEST_CASE(tests_blackmanStd32fc, nmppsblackmanStd32fc_check_rewrite);
    RUN_TEST_CASE(tests_blackmanStd32fc, nmppsblackmanStd32fc_calculation);
}
